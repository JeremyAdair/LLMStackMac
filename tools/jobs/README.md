...
```bash
./tools/jobs/backup --keep-days 14
```

Validate a specific backup before restore:

```bash
./tools/jobs/restore-check ./data/backups/backup-YYYYMMDD-HHMMSS
```

## Notes

- These scripts are for Docker-based LLMStack backups on this host.
- `restore-check` does not restore data; it only validates backup integrity.
