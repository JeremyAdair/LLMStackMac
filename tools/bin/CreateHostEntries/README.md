...
```bash
./tools/bin/CreateHostEntries/llm-hosts-update --check
```

Apply/update `/etc/hosts`:

```bash
./tools/bin/CreateHostEntries/llm-hosts-update --apply
```

Custom domain/IP:

```bash
LLMSTACK_DOMAIN=llmstack.lan LLMSTACK_HOSTS_IP=127.0.0.1 ./tools/bin/CreateHostEntries/llm-hosts-update --apply
```
