...
if [[ $# -lt 1 ]]; then
  echo "Usage: llm ocr run <filename>" >&2
  exit 1
fi
...
printf '{\n  "input_file": "%s",\n  "text_output": "%s",\n  "chunk_glob": "%s/%s-*.txt",\n  "status": "processed"\n}\n' \
  "${input_name}" \
  "data/ocr/processed/rawtext/${base_name}.txt" \
